...
**Работают интерфейс и погодный backend участника №2; модель участника №1 ещё нужно подключить.**
Исходные CSV скопированы в локальную игнорируемую папку `data/scada/`, реальные
архивы NOAA GFS сохраняются в `data/weather/`. Есть агент Responses API, проверки,
сохранение версий и replay. Живой OpenAI-вызов требует API-ключа, а прогноз мощности
и январские метрики — ML-модели команды.
...
| События агента, replay и проверка обновлений | Backend реализован; прогноз мощности ожидает модель №1 |
| Обучение и реальные февральские прогнозы мощности | Зона участника №1; модель пока не подключена |
...
Результат проверки backend и UI на Linux: **131 passed, 1 skipped**.
В тестах есть проверки настоящего локального архива GFS; модель и Responses API
проверяются на тестовых подстановках. Реальный smoke-test прогноза мощности
пропускается с объяснением, пока не задан артефакт модели команды.
