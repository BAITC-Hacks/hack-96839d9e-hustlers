...
Модуль участника №1 подключён как `WINDOPS_ML_MODULE=windops.ml.plugin`:
отдельные CatBoost обучены, январский backtest и полный февральский replay
выполнены на реальных данных. Команды и результаты — [ML.md](ML.md).
...
Все предоставленные и скачанные данные находятся в **`data/`**;
исходные CSV и архив добавлены командой в Git:
...
Сквозной реальный прогноз, replay и обновление проверены с обученной моделью №1
в детерминированном режиме; отчёт — `data/ml/backend_verification.json`.
Живой LLM-сценарий требует отдельной проверки и API-ключа.
