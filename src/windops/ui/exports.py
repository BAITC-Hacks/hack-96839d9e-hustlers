...
def passport_json(bundle) -> bytes:
    ...
    payload = {"forecast_id": bundle.forecast_id, "site_ids": bundle.site_ids,
               ...
               "revises_forecast_id": bundle.revises_forecast_id,
...
def february_csv(bundles, site_ids, timezone, rule="daily_previous_day_23", final=False) -> bytes:
    """Day D uses steps 1–24 of original D−1 23:00; updates never replace it."""
    if rule != "daily_previous_day_23":
    ...
    for bundle in bundles:
        if final:
            _require_final(bundle)
        for member in bundle.members or [bundle]:
            if member.revises_forecast_id is None:
                originals.append(_rows(member, final))
    if not originals:
        raise ExportBlocked("Нет исходных ежедневных выпусков для таблицы февраля.")
    rows = pd.concat(originals, ignore_index=True)
    ...
    expected_origin = rows["_target"].dt.tz_convert(timezone).dt.normalize() - pd.Timedelta(hours=1)
    chosen = rows.loc[rows["_issued"].eq(expected_origin) & rows["horizon_step"].between(1, 24)].copy()
